...
const colStringToNumber1 = (col) => {
	...
	let column = 0;
	for (let i = 0; i < length; i++) column += (col.charCodeAt(i) - 64) * Math.pow(26, length - i - 1);
	return column;
};
/**
* Converts column letter to number
...
const colStringToNumber2 = (col) => {
	const A = "A".charCodeAt(0), radix = "Z".charCodeAt(0) - A + 1;
	let i, l, chr, sum = 0;
...
//#endregion
//#region src/A1Error.ts
/**
* @fileOverview A1 notation errors
*/
var A1Error = class extends Error {
...
var A1 = class A1 {
	...
	*/
	static _parse(a1, converter) {
		const [, cs, rs, ceMatch, reMatch] = a1.toUpperCase().match(this._reg) ?? [];
		const ce = ceMatch || cs;
		const re = reMatch || rs;
	...
	*/
	_initObject(options) {
		...
		const getValue = (some, canBeLetter = true) => {
			...
			if (canBeLetter && isLetter(some)) return A1._A1Col(some, this._converter);
			return 0;
		};
		/**
		* Define start range
	...
	*/
	_initNumber(...args) {
		...
		this._colStart = col;
		this._rowStart = row;
		this._colEnd = col + nCols - 1;
		this._rowEnd = row + nRows - 1;
	}
	/**
	*	It handles cases:
	...
	*/
	...
	constructor(...args) {
		...
		if (something && type(something) === "object") this._initObject(something);
		else if (isNumber(something)) this._initNumber(...args);
		else if (isString(something)) this._initString(...args);
	...
	*/
	_setFields(val, field, axis, canBeLetter = true) {
		...
		else if (canBeLetter && isLetter(val)) this[field] = A1._A1Col(val, this._converter);
		else throw new A1Error(val).u();
		if (this[`_${axis}Start`] > this[`_${axis}End`]) throw new A1Error(`The first column or row can't be bigger than the last, got: ${val}`);
		return this;
	}
	/**
